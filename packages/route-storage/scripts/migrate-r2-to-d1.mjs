...
function buildSql(routes, maps) {
	...
	for (const map of maps) {
		...
		statements.push(
			`INSERT INTO maps (id, name, owner, type, text, background, spots, x, y, image, video, points, updated_at) VALUES (${sqlValue(map.id)}, ${sqlValue(map.name)}, ${sqlValue(map.owner)}, ${sqlValue(map.type)}, ${sqlValue(text)}, ${sqlValue(map.background)}, ${sqlValue(map.spots)}, ${sqlValue(map.x)}, ${sqlValue(map.y)}, ${sqlValue(map.image)}, ${sqlValue(map.video)}, ${sqlValue(points)}, CURRENT_TIMESTAMP);`,
		);
	}
